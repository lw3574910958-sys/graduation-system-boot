...
class TopicControllerTest {
    ...
    void setUp() {
        ...
        mockTopicVO.setTitle("基于Spring Boot的毕业设计管理系�?);
    ...
    void getTopicPage_ShouldReturnTopicPage() throws Exception {
        ...
        mockMvc.perform(get("/api/topics/page")
                ...
                .andExpect(jsonPath("$.data.records[0].title").value("基于Spring Boot的毕业设计管理系�?))
                .andExpect(jsonPath("$.data.records[0].teacherName").value("李教�?))
    ...
    void getTopicById_ExistingTopic_ShouldReturnTopicVO() throws Exception {
        ...
        mockMvc.perform(get("/api/topics/{id}", 1L))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.data.title").value("基于Spring Boot的毕业设计管理系�?))
                .andExpect(jsonPath("$.data.description").value("设计并实现一个完整的毕业设计管理系统"))
                .andExpect(jsonPath("$.data.teacherName").value("李教�?));
    }
    ...
    void getTopicById_NonExistingTopic_ShouldReturnError() throws Exception {
        // Given
        when(topicService.getTopicById(999L)).thenThrow(new RuntimeException("课题不存�?));
